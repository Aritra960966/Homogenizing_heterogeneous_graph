...
--------------------
Downloads ALL raw datasets for the RAHGH experiment pipeline.

Datasets covered:
  Heterogeneous benchmarks : DBLP, ACM, IMDB  (via PyG)
  Large-scale              : OGBN-MAG          (via OGB)
  Social / recommendation  : Amazon, LastFM    (via HGB benchmark)
  Optional                 : Yelp, Freebase    (via HGB benchmark)
...
Requirements
------------
    pip install torch torch-geometric ogb requests tqdm
"""
...
def _extract(archive: Path, dest_dir: Path) -> None:
    """Auto-extract zip / tar.gz / tar.bz2."""
...
def download_dblp() -> bool:
    """
    DBLP via PyTorch Geometric HAN dataset loader.
    Stored at: data/raw/dblp/
    """
    log.info("=== DBLP ===")
    dest = RAW / "dblp"
...
def download_acm() -> bool:
    """
    ACM via PyTorch Geometric HAN dataset loader.
    Stored at: data/raw/acm/
    """
    log.info("=== ACM ===")
    dest = RAW / "acm"
    dest.mkdir(parents=True, exist_ok=True)
...
def download_imdb() -> bool:
    """
    IMDB via PyTorch Geometric IMDB dataset loader.
    Stored at: data/raw/imdb/
    """
    log.info("=== IMDB ===")
    dest = RAW / "imdb"
    dest.mkdir(parents=True, exist_ok=True)
...
def download_ogbn_mag() -> bool:
    """
    OGBN-MAG via OGB library.
    Large dataset (~1.9M nodes). Stored at: data/raw/ogbn_mag/
    """
    log.info("=== OGBN-MAG ===")
    dest = RAW / "ogbn_mag"
    dest.mkdir(parents=True, exist_ok=True)
...
def download_amazon() -> bool:
    """
    Amazon product graph (Electronics subset).
    Source: HGB benchmark / PyG Amazon dataset.
    Stored at: data/raw/amazon/
    """
    log.info("=== Amazon ===")
    dest = RAW / "amazon"
    dest.mkdir(parents=True, exist_ok=True)
    ...
    try:
        ...
        # Computer subset as heterogeneous-friendly split
    ...
    # Fallback: HGB benchmark Amazon
    return _download_hgb_dataset("Amazon", dest)


def download_lastfm() -> bool:
    """
    LastFM user-artist-tag heterogeneous graph.
    Source: HGB benchmark.
    Stored at: data/raw/lastfm/
    """
    log.info("=== LastFM ===")
    dest = RAW / "lastfm"
    dest.mkdir(parents=True, exist_ok=True)
...
def download_yelp() -> bool:
    """
    Yelp business-review heterogeneous graph.
    Source: HGB benchmark.
    Stored at: data/raw/yelp/
    """
    log.info("=== Yelp ===")
    dest = RAW / "yelp"
    dest.mkdir(parents=True, exist_ok=True)
...
def download_freebase() -> bool:
    """
    Freebase knowledge graph subset.
    Source: HGB benchmark.
    Stored at: data/raw/freebase/
    """
    log.info("=== Freebase ===")
    dest = RAW / "freebase"
    dest.mkdir(parents=True, exist_ok=True)
...
# ── HGB benchmark fallback downloader ──────────────────────────────────────────

# HGB (Heterogeneous Graph Benchmark) dataset registry
# Source: https://github.com/THUDM/HGB
...
def _download_hgb_dataset(name: str, dest: Path) -> bool:
    """Download a dataset from the HGB benchmark mirror."""
...
def write_manifest(results: dict) -> None:
    """Write download_manifest.json to data/raw/."""
