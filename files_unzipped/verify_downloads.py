...
verify_downloads.py
-------------------
Verifies that all raw datasets have been downloaded correctly.
Prints node/edge counts, relation types, and confirms the .done marker.

Usage
-----
    python scripts/verify_downloads.py
    python scripts/verify_downloads.py --dataset dblp acm
"""
...
def check_marker(name: str) -> bool:
    ...
    if marker.exists():
        ...
        return True
    log.warning(f"  ✗ .done marker missing — download may be incomplete")
    return False
...
def load_and_report_ogb(name: str, module: str, cls: str, ogb_name: str) -> None:
    try:
        import importlib
        mod = importlib.import_module(module)
        DatasetCls = getattr(mod, cls)
...
def main():
    ...
    parser.add_argument("--dataset", nargs="+",
                        choices=["dblp","acm","imdb","ogbn_mag","amazon","lastfm","yelp","freebase"],
                        help="Datasets to verify (default: all)")
    args = parser.parse_args()

    datasets = args.dataset or ["dblp","acm","imdb","ogbn_mag","amazon","lastfm","yelp","freebase"]
