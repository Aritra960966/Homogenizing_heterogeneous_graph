"""
IMDB NC — HGB splits, GPU-optimized, 100 hyperparameter combos.
Run: python HGB/scripts/run_nc_imdb.py
"""
...
data = _get_loader('imdb')
data['name'] = 'imdb'
data['train_indices'] = np.load(os.path.join(_HERE, '..', 'splits', 'nc', 'imdb', 'train_indices.npy'))
data['test_indices'] = np.load(os.path.join(_HERE, '..', 'splits', 'nc', 'imdb', 'test_indices.npy'))

from src.train import run_nc
run_nc('imdb', os.path.join(_HERE, '..', 'results', 'nc', 'imdb'))
